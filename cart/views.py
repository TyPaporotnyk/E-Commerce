...
def cart_page(request):    
    ...
    products_quantity = [int(request.session['cart_products'][key]) for key in list(products_ids.keys())]
...
def make_order(request):
    ...
    for product in products:
        o_product = OrderProduct.objects.create(
            product=product,
            quantity=cart_products[str(product.id)]
        )
        o_product.save()
    ...
    return JsonResponse({'success': 'Order has been created successfully', 'order_id': order.id})
